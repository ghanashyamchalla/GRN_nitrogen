...
%% Special case, 0 bootstraps
    if(nbootstraps(1) == 0)                    %Just do lasso not bolasso
        ...
        %fprintf(1, 'bootstrap 0, (lasso only)...\n');
...
%%
    ...
    %non-zero given l1 regularizer lambda(i).
        ...
        criticalPoints = recoverLambda(Xsamp,ysamp,Wfull)'; %in ascending order of magnitude.
        tooBig = lambda > criticalPoints(end);%can't interpolate outside of the range of criticalPoints   
        Winterp = interp1q(criticalPoints,Wfull,lambda(~tooBig)');
        nzeroPage = Winterp ~=0;
        nzeroPage = [nzeroPage;false(sum(tooBig),d)]; % since, if lambda > lambda_max, all weights 0. 
...
%%
    ...
    % Wfull, as returned by lars. 
        W = Wfull';
        % dif = bsxfun(@minus,y,X*W);
        dif = y * ones(1, size(W, 2)) - X*W;
        lambdaVals = 2*max(abs(X'*dif),[],1);
    end
%%
